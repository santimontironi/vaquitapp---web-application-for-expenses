...
**Carpeta `backend/utils/` (agregada 2026-06-10):** contiene `emailTemplates.js`, que exporta `getConfirmAccountEmailHtml(username, urlConfirmed)`. Genera el HTML del correo de confirmacion de cuenta con estilos inline (tablas, max-width 600px), siguiendo la paleta `#210B2C`/`#BC96E6`/`#FFD166` y tipografia "Outfit". `auth.controller.js` lo importa y lo usa como `html` en `mailOptions` del registro, manteniendo `text` como fallback plano.

**Why:** el proyecto esta en desarrollo activo. La feature de gastos fue completada en una iteracion anterior. El README fue actualizado el 2026-06-10 para reflejar el rediseño visual (paleta, fuente, animaciones Motion), la estructura completa de carpetas frontend (hooks/services/types/utils ya no estan vacias), y la nueva carpeta `backend/utils/` con la plantilla HTML del email de confirmacion.
